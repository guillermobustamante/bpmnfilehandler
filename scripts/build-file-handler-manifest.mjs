...
if (!appBaseUrl) {
  console.error("Usage: npm run manifest -- https://your-app.azurewebsites.net [outputPath] [.bpmn,.drawio]");
  process.exit(1);
}
...
const manifest = requestedExtensions.map((extension) => buildFileHandler(extension, handlerIds[extension], appBaseUrl));
...
function buildFileHandler(extension, fileHandlerId, baseUrl) {
  const spec = getExtensionSpec(extension);
  const fileIcon = JSON.stringify(buildIconSet(baseUrl, spec.assetPrefix, "file"));
  const appIcon = JSON.stringify(buildIconSet(baseUrl, spec.assetPrefix, "app"));
...
function buildIconSet(baseUrl, assetPrefix, iconType) {
  return {
    svg: `${baseUrl}/assets/${assetPrefix}-${iconType}.svg`,
    png1x: `${baseUrl}/assets/${assetPrefix}-${iconType}-32.png`,
    "png1.5x": `${baseUrl}/assets/${assetPrefix}-${iconType}-48.png`,
    png2x: `${baseUrl}/assets/${assetPrefix}-${iconType}-64.png`
  };
}
