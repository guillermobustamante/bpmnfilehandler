...
- `preview` opens a read-only BPMN rendering inside the SharePoint/OneDrive iframe.
- `open` opens the full editor in a new browser tab and saves changes back to the selected DriveItem.
...
5. MSAL signs in the user with a login hint from the activation payload.
